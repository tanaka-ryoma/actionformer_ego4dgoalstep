...
def train_one_epoch(
    ...
    for iter_idx, video_list in enumerate(train_loader, 0):
        ...
        losses = model(video_list)
        print("yes3",losses)
        final_loss = losses['final_loss'] / accum_iter
        final_loss.backward()
        # gradient cliping (to stabilize training if necessary)
        if clip_grad_l2norm > 0.0:
            torch.nn.utils.clip_grad_norm_(
    ...
    return losses_tracker['final_loss'].val
...
def valid_one_epoch(
    ...
    for iter_idx, video_list in enumerate(val_loader, 0):
        ...
        with torch.no_grad():
            output_tuple = model(video_list)
            
            #print(type(video_list))

            # # 損失を計算
            # targets = torch.cat([vid['labels'] for vid in video_list], dim=0)
            # predictions = torch.cat([out['scores'] for out in output], dim=0)
            # # 予測とターゲットを使ってロスを計算
            # loss = loss_fn(predictions, targets)
            # total_loss += loss.item()
            output, loss = output_tuple
            val_loss = loss['final_loss']
            # print("val_loss", val_loss)

            # unpack the results into ANet format
            num_vids = len(output)

            #print("output", output, "num_vids", num_vids) output[1,1024,2304]
            ...
            for vid_idx in range(num_vids):
                if output[vid_idx]['segments'].shape[0] > 0:
                    ...
                    results['ground truth'].append(video_list[vid_idx]['labels'])
        ...
        if (iter_idx != 0) and iter_idx % (print_freq) == 0:
            ...
            # print("loss", final_loss, cls_loss, reg_loss)

    # gather all stats and evaluate
    results['t-start'] = torch.cat(results['t-start']).numpy()
    results['t-end'] = torch.cat(results['t-end']).numpy()
    results['label'] = torch.cat(results['label']).numpy()
    results['ground truth'] = torch.cat(results['ground truth']).numpy()
    results['score'] = torch.cat(results['score']).numpy()

    print("t-start,end",results['t-start'][:20],results['t-end'][:20])
    print("label", results['label'][:20])
    print("label_len",len(results['label']))
    print("gt", results['ground truth'][:20])
    print("gt_len", len(results['ground truth']))
    print("score", results['score'][:20])
