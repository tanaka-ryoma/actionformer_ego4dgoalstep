...
class PtTransformer(nn.Module):
    ...
    def forward(self, video_list):
        ...
        for i, feat in enumerate(feats[:20]):
            print(f"feat_inPt1[{i}]", feat.shape)

        fpn_feats, fpn_masks = self.neck(feats, masks)
        for i, feat in enumerate(fpn_feats[:20]):
            print(f"feat_inPt2[{i}]", feat.shape)
        ...
        if self.training:
            ...
            # print("gt_seg",gt_segments)
            print("gt_label2",gt_labels)
            ...
            gt_cls_labels, gt_offsets = self.label_points(
                points, gt_segments, gt_labels)
            print("gt_label3", gt_labels)

            losses = self.losses_val(
                fpn_masks, out_cls_logits, out_offsets, gt_cls_labels, gt_offsets
            )
